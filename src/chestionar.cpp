...
Chestionar::Chestionar() {
    ...
    std::shuffle(intrebarile_totale.begin(), intrebarile_totale.end(), default_random_engine());
    // for (int i =0; i<intrebarile_totale.size(); i++) {
    //     std::cout<<"da";
    // }
    for (int i=0; i<=5; i++) {
    ...
    for (const Intrebare& i : set_intrebari) {
        ...
        if (r == i.getRaspuns()) {
            ...
            std::cout<<std::endl<<"Gresit!"<<std::endl<<"Raspunsul corect este:"<<std::endl<<i.getRaspuns()<<std::endl;
        }
    }
    this->punctaj = P;
};
